...
; Compression strength: extreme (maximum) or 1-9
...
class BackupConfig:
    ...
    def _parse(self) -> None:
        ...
        self.comp_level       = g.get("compression_level",      "extreme").lower()
    ...
    def should_skip_file(
        ...
        for pat in self.patterns:
            if fnmatch.fnmatch(name, pat):
                return True, f"matches exclusion pattern '{pat}'"
...
class BackupEngine:
    ...
    def _probe_file(filepath: str) -> Optional[str]:
        """
        Read the first 64 KB to verify the file is currently accessible.
    ...
    def _archive_add(
        ...
        """
        Add a single file to an open TarFile object.
        Returns None on success, or a descriptive error string on failure.

        The TarInfo is obtained first so the metadata is captured at this
        instant; then the file is streamed in via an explicit fileobj to avoid
        tarfile recursing into sub-paths (recursive=False is not available on
        addfile).
        """
        try:
            ...
            with open(filepath, "rb") as fh:
                tar.addfile(tarinfo=info, fileobj=fh)
            return None
    ...
    def _process_file(
        ...
        for attempt in range(1, 3):     # attempts 1 and 2
            ...
            # ── Archive write ─────────────────────────────────────────────────
            write_err = self._archive_add(tar, filepath)
            if write_err:
                if attempt == 1:
                    self.log.warning(
            ...
            # ── Success ───────────────────────────────────────────────────────
            chk = sha256_file(Path(filepath)) or "unavailable"
